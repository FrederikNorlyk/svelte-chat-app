...
export class UserClient extends DatabaseClient {
    ...
    public async listAllOtherUsers(currentUserId: number) {
        const result = await this.pool.sql `SELECT * FROM chat_users WHERE id != ${currentUserId}`;

        return result.rows.map((row) => this.serialize(row));
    }

    public async create(name: string) {
        const result = await this.pool.query(`INSERT INTO chat_users(name) VALUES($1) RETURNING *`, [name])
        const row = result.rows[0]
        return this.parse(row)
    }

    public async getByName(name: string) {
        const result = await this.pool.sql `SELECT * FROM chat_users WHERE name ILIKE ${name}`;
